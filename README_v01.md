...
> **Filename convention used in this README**  
> This document assumes the final reviewer-package names `clean_feature_generator_fixed_BL39.py` and `run_SOAP_benchmark.py`. If local working copies still use earlier names, rename them before distribution so that the repository and README remain consistent. Test-only prefixes such as `PRETEST_DEBUG_SOAP_` should also be removed from the final distributed outputs after numerical verification.
...
The reviewer-oriented SOAP workflow uses the following three **precomputed processed inputs**:
...
It is itself a precomputed upstream output. The minimal reviewer workflow does **not** regenerate this file from raw trajectories.
...
The `oof_predictions_existing.csv` file supplies the grouped OOF fold mapping used for the matched comparison. In the recommended reviewer workflow, this file is generated by `run_SOAP_benchmark.py`; therefore, the SOAP benchmark should be executed before the matched monolithic control unless an identical verified fold-mapping file is already supplied.
...
The minimal reviewer workflow uses three supplied precomputed inputs:
...
The complete raw trajectory and snapshot archive are intentionally omitted from the minimal reviewer package.
...
The processed-input SOAP benchmark does not require regeneration of SOAP descriptors from raw atomic structures. Therefore, packages required only for upstream raw SOAP construction are not necessary for the minimal reviewer workflow.
...
The distributed `requirements.txt` should be checked against the final set of reviewer-facing scripts before submission.
...
“Frenkel–Ladd-anchored thermal backbone learning for admissibility-aware free-energy reconstruction under descriptor aliasing,” manuscript submitted for publication (2026).
