...
OUT_PRED = TABLE_DIR / "response_only_predictions.csv"
OUT_BY_ST = TABLE_DIR / "response_only_summary_by_system_T.csv"
OUT_GLOBAL = TABLE_DIR / "response_only_summary_global.csv"
OUT_BY_T = TABLE_DIR / "response_only_summary_fixedT_across_systems.csv"
OUT_BY_SYS = TABLE_DIR / "response_only_summary_within_system_across_T.csv"
...
def main():
    ...
    if not feature_cols:
        raise ValueError("No response-only feature columns were found in input data")

    model = Pipeline(
        [
            ("scaler", StandardScaler()),
            ("krr", KernelRidge(alpha=1e-2, kernel="rbf", gamma=0.2)),
        ]
    )

    df["dFpred_response_only"] = fit_predict_group_cv(
        df=df,
    ...
    df["Fpred_response_only"] = df["F0_eVatom"] + df["dFpred_response_only"]
    ...
    by_st, global_df = summarize(df, "Fpred_response_only", "response_only")
    ...
    by_st, global_df = summarize(df, "Fpred_response_only", "response_only")
    by_T = summarize_fixedT_across_systems(df, "Fpred_response_only", "response_only")
    by_sys = summarize_within_system_across_T(df, "Fpred_response_only", "response_only")
